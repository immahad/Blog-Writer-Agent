...
spec = importlib.util.spec_from_file_location("blog_agent_backend", "blog-agent-backend.py")
blog_agent_backend = importlib.util.module_from_spec(spec)
sys.modules["blog_agent_backend"] = blog_agent_backend
spec.loader.exec_module(blog_agent_backend)
app = blog_agent_backend.app
    ...
    if not past_files:
        ...
        if st.button("📂 Load selected blog"):
            if selected_md_file:
                ...
                st.session_state["logs"] = loaded_logs
...
if run_btn:
    ...
    for kind, payload in try_stream(app, inputs):
        if kind in ("updates", "values"):
            ...
            if plan_obj:
                blog_title = plan_obj.blog_title if hasattr(plan_obj, "blog_title") else plan_obj.get("blog_title", "blog")
                safe_title = re.sub(r'[\\/*?:"<>|]', "_", blog_title)
                ...
                ev_list = out.get("evidence") or []
                evidence_dict = []
                for e in ev_list:
                    if hasattr(e, "model_dump"):
                        evidence_dict.append(e.model_dump())
                    elif isinstance(e, dict):
                        evidence_dict.append(e)
                    else:
                        try:
                            evidence_dict.append(json.loads(json.dumps(e, default=str)))
                        except:
                            pass
                ...
                try:
                    Path(f"{safe_title}.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
                except Exception as e:
                    log(f"Failed to save metadata JSON: {e}")
